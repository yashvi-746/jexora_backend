...
exports.getRFQItems = async (req, res) => {
  try {
    const rfqId = req.params.id;
    const items = await RFQItem.find({ rfqId }).populate('productId', 'name sku price');
    res.json(items);
  } catch (error) {
    res.status(500).json({ message: error.message });
  }
...
exports.getRFQById = async (req, res) => {
  try {
    const rfqItem = await RFQ.findOne({ _id: req.params.id }).populate("suppliers", "name email");
...
exports.deleteRFQ = async (req, res) => {
  try {
    const rfq = await RFQ.findOneAndDelete({ _id: req.params.id });
    if (!rfq) return res.status(404).json({ message: "RFQ not found" });
    // Also delete associated items
    await RFQItem.deleteMany({ rfqId: req.params.id });
